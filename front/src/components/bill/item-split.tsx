...
export function ItemSplit({ bill }: ItemSplitProps) {
  ...
  const paidSet = new Set(bill.paidItemIds ?? []);
  const lockedSet = new Set(bill.lockedItemIds ?? []);
  const groups = groupByCategory(bill.items);
  const hasSelection = claimedItemIds.length > 0 || sharedItemIds.length > 0;

  // Count unclaimed items (exclude paid/locked)
          ...
          <div key={category}>
            ...
            <ul role="list">
              {groupItems.map((item) => {
                ...
                const isDisabled = isPaid || isLocked;

                return (
                  <li key={item.id} className="group">
                    {isPaid ? (
                      <div className="flex items-center gap-3 rounded-soft px-3 py-3 opacity-45">
                        <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-soft border-2 border-success bg-success text-white">
                          <svg
                            width="14"
                            height="14"
                            viewBox="0 0 24 24"
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-baseline justify-between">
                            ...
                            <CurrencyDisplay
                              ...
                              size="sm"
                              className="shrink-0 ml-3 text-muted line-through"
                            />
                          </div>
                        </div>
                      ...
                      `}
                      >
                        {/* Claim area — entire left section is clickable */}
                        <button
                          type="button"
                          role="checkbox"
                          aria-checked={isClaimed}
                          aria-label={`Claim ${item.name}${item.quantity > 1 ? ` ×${item.quantity}` : ""}`}
                          onClick={() => toggleClaimItem(item.id)}
                          className="flex min-h-[44px] flex-1 items-center gap-3 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500/20 rounded-soft"
                        >
                          {/* Checkbox indicator */}
                          <span
                            className={`
                        ...
                        <button
                          ...
                          className={`
                          shrink-0 rounded-full px-3 py-2 min-h-[44px]
                          text-xs font-medium
                          transition-all duration-150
